...
class DrainHandler:
    def __init__(self, config):
        self.name = getValue(config, 'name', '')
        self.minimum_length = config['minimum-length']
        self.drain_file_name = getValue(config, 'snapshot-file', f"{self.name}.bin")
        self.words = getValue(config, 'words', [])
        self.ignore_words = getValue(config, 'ignore-words', [])
        self.custom_masking_words = getValue(config, 'custom-masking-words', [])
        self.match_rate = getValue(config, 'match-rate', 0)
        self.match_max_count = getValue(config, 'match-max-count', 0)
        self.mode = getValue(config, 'mode', 'training')
        self.write_file_flag = getValue(config, 'write-file-flag', False)
        ...
        self.similarity_threshold = getValue(config, 'similarity-threshold', default_sim_th)
        self.depth = getValue(config, 'depth', default_depth)
        self.compress_state = getValue(config, 'compress-state', default_compress_state)
        self.parametrize_numeric_tokens = getValue(config, 'parametrize-numeric-tokens', default_parametrize_numeric_tokens)
    ...
    def training(self, line, monitoringfilename="", offset=0):
        ...
        if self.write_file_flag:
            try:
                with open(f"{self.file_fullpath}\\..\\output\\training_text\\{self.name}.txt", 'a', encoding='UTF8') as f:
                    f.writelines(f"{input_line}\n")
    ...
    def both(self, line, match_line, offset=0, monitoringfilename=""):
        self.monitoring_file_name = monitoringfilename
        input_line = self.get_training_data(match_line)
        if input_line == "":
            return offset
        ...
        if result["change_type"] != "none":
            ...
            if result["change_type"] == "cluster_created":
                try:
                    with open(f"{self.file_fullpath}\\..\\output\\inference\\{self.name}.txt", 'a', encoding='UTF8') as f:
                        f.writelines(f"[#{result['cluster_id']}][{monitoringfilename}][{offset}]: {line}\n")
                except Exception as e:
                    print("Exception : " + str(e))
