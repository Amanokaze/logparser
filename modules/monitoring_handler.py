...
class MonitoringHandler():
    def initConfig(self, config):
        ...
        self.monitoring_directory = getValue(monitoring, 'directory', '.\\')
        self.monitoring_pattern = getValue(monitoring, 'pattern', 'none')
        self.monitoring_file = getValue(monitoring, 'file', None)
        self.monitoring_extension = getValue(monitoring, 'extension', 'log')
        self.monitoring_filename = None

        self.no_datetime_log = getValue(monitoring, 'no-datetime-log', 'separate')
        ...
        def execute_both(f):
            ...
            if self.no_datetime_log == "separate":
                for line in f.readlines()[last_offset:]:
                    ...
                    input_line_data = line_data.rstrip()
                    input_line = line.rstrip()
                    self.last_offset = self.drain_handler.both(input_line, input_line_data, self.last_offset, self.monitoring_filename)
            elif self.no_datetime_log == "streaming":
                input_line_data = str()
                input_line = str()
                for line in f.readlines()[last_offset:]:
